...
int main()
{
    ...
    char filenameIN[20],filenameIN2[20],filenameOUT[20];
    int data1, data2, Dt_list[100][2],N,i=0;
	double totalx=0,totaly=0,ratax,ratay,varx=0,vary=0,kor=0,stx,sty,rxy,b1,b0;
	string stringline, stringline2, Dt_stringline[100],Dt_stringline2[100];
    int eof();
...
cout << " Enter filename to be opened ";
    ...
    if (infile.is_open())
    {
        while ( getline (infile,stringline2) )
        {
            cout << stringline2 << '\n';
            Dt_stringline2[i]=stringline2;
            i++;
        }
        infile.close();
    }
